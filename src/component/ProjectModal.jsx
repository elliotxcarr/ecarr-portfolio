...
const Modal =({open, id, selectedProject, description, img, link, fade, onClose}) =>{

    if(!open) return null

    return(

        <motion.div className={`fixed inset-0 bg-dark bg-opacity-25 backdrop-blur-sm flex justify-center items-center z-100`}
        initial={{opacity:0}}
        exit={{opacity:0}}
        animate={{opacity:1}}
